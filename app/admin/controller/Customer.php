...
class Customer extends Base
{
    ...
    public function index(Request $request)
    {
        ...
        if($keywords==""){
            $data=CustomerModel::field('id,username,phone,gender,address')->select();
            $res=CustomerModel::limit($limit)->page($page)->select();
        }else{
            $data=CustomerModel::where('username','like',"%{$keywords}%");
            $res=CustomerModel::where('username','like',"%{$keywords}%")->limit($limit)->page($page)->select();
        }
    ...
    public function save(Request $request)
    {
        ...
        $id=CustomerModel::create($data)->getData('id');
        if (empty($id)){
...
//            return  json(['code'=>200,"msg"=>"添加成功！","id"=>$id]);
    ...
    public function read($id)
    {
        ...
        $data=CustomerModel::field('id,username,password,phone,gender,address')->find($id);
        if (empty($data)){
